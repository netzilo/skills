...
| **Endpoint Security Settings** | device hygiene; **all enabled items must pass** | Antivirus (installed, active, up to date — Windows), Firewall (Windows, macOS), Disk Encryption (Windows, Linux, macOS), Screen Lock (password-protected lock — Windows, Linux, macOS), OS Updates (auto-update on and current — Windows, Linux, macOS) | per item |
| **Advanced Endpoint Settings** | presence/absence indicators; **all enabled items must pass** | Enterprise Workspace (device must be using it — Windows), Enterprise Browser (Windows, macOS), Virtual Device (must **not** be a VM — Windows, Linux, macOS), Device Integrity (not rooted/jailbroken/debugged — Windows, Linux, macOS, iOS, Android), Registry Key & Value (Windows; All/Any; hive HKLM/HKCU/HKCR/HKCC/HKU, key, value; wildcards), File & Folder (All/Any; per OS path + optional content regex), Running Processes (All/Any; per OS path patterns) | per item |
...
- Security Settings items are OS-specific; a check with Antivirus enabled will fail every
  macOS/Linux device — put per-OS checks in separate posture checks.
- Virtual Device: blocks VMs, including servers. Never attach it to a policy whose source
  group contains servers or routing peers.
...
| Security Settings check fails on a compliant device | which indicator is red on the peer detail page? Antivirus "up to date" and OS Updates depend on the OS reporting; Screen Lock requires a password-protected lock | fix the device setting, or remove the item |

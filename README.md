...
|---|---|---|
| [`netzilo-admin`](netzilo-admin/) | 2.11.0 | Install, configure, run and troubleshoot Netzilo end to end: server (on-prem, AWS, Azure), identity and SSO, clients on every OS, network policy, AI security (AIDR), REST API automation, connectivity diagnosis, Windows hosts, the secure Workspace, log interpretation, bounded remediation workflows, onboarding and rollout, incident response and recovery, plan and billing limits, data-handling answers for security reviews, an end-user guide to publish, and support escalation. |
